...
Page({
  ...
  data: {
	...
	radioChange(e){
		...
		this.getRadioSecondList(d[1]);
	},
	getRadioSecondList(code){
		if(!this.data.secondList[""+code]){
			$.ajax({
				url:router.D_GET+'?pid='+code
			}).then(res=>{
				this.setData({
					["otherForm.typeParent"]:code,
					["otherForm.typeChild"]:"",
					["secondList."+code]:res.data
				});
			})
		}else{
			this.setData({
  ...
  onLoad: function (options) {
		...
		eventChannel.on('fromData', ({data})=>{
			const {color,name,subtitle,discount,price,sort,size,content,id,isEvent}=data;
			const typecode = data.typecode.split(",");
			prod_id = id;
			let obj = {
				typeParent:typecode[0],
				typeChild:typecode[1],
				cover:[],
				isEvent
			}
			const cover = data.cover.split(",");
			for (let index = 0; index < cover.length; index++) {
				obj.cover.push({
					url:$.host+cover[index],
					urlapi:cover[index]
				})
			}
			$.ajax({
				url:router.D_GET+'?pid='+typecode[0].slice(0,2)+"0000"
			}).then(res=>{
				this.setData({
					firstList:res.data,
					otherForm:obj,
			...
			$.ajax({
				url:router.D_GET+'?pid='+typecode[0]
			}).then(res=>{
				this.setData({
					["secondList."+typecode[0]]:res.data,
				});
			})
    })
  },
